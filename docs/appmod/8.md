...
1. Return to the Visual Studio Code (VS Code) application. It should already be signed in and authenticated with the WCA service on IBM Cloud — if it's not, follow the instructions in the <a href="https://ibm.github.io/wca-l4/saas/4/#iii-install-the-extension" target="_blank">**IBM Cloud (SaaS) Configuration**</a> lab.
    ...
    - Ensure that the **Explorer** tab is cleared of any previously-opened folders or files before continuing.
    - With the VS Code **Explorer** tab selected and open, click **File** > **Close Folder** from the top taskbar.
...
3. Next, participants must download the WebSphere Application Server (WAS) assets that are to be modernized. The assets will be retrieved from the following GitHub repository:

    !!! note "GitHub URL"
        ``` shell
        https://github.ibm.com/cdbienko/wca-assets-2025.git
        ```
    
    - Open the **Explorer** tab from the VS Code interface
    - Click the **Clone Repository** button to open a pop-up window in the top-center of the VS Code interface
    - Paste in the full **GitHub URL** (above) and then press ++return++ (or click the **Clone from URL** button just below the pop-up window)
    - VS Code will ask you to select where on the local machine to clone the repository: select your Desktop (or another location you can easily find later)
    - Once cloned, the `wca-assets-2025` folder will automatically open within **Explorer**
...
4. Participants now need to build the application *Project*. Open a **Terminal** within VS Code (if you haven't already) by clicking **Terminal** > **New Terminal** from the top taskbar.

    - Navigate using the Terminal to the folder where `wca-assets-2025` was cloned in *Step 3* and drill down into `/1-java-modernization/modresorts-twas-j8/was_dependency` directory (substitute the `<REPLACE>` placeholder with the path to *your* locally-cloned directory):
    ``` shell
...
6. With the `was_dependency` folder now prepared with the necessary drivers, you are ready to *build* and *deploy* the application project into a live web application
    ...
    - Below the list of assets belonging the `modresorts-twas-j8` directory, locate and toggle the **LIBERTY DASHBOARD** drop-down panel amongst the tabs
