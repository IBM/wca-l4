...
1. First create the prerequisite project namespace by executing the following instruction with a Terminal console:

    ``` shell
    oc new-project cert-manager-operator
    ```

    ![](_attachments/onprem-5.1.png){: loading=lazy width="400"}
...
2. Afterwards, return to your web browser and open the **OCP Dashboard** (<a href="https://ibm.github.io/wca-l4/on-premises/2/#ii-accessing-the-cluster" target="_blank">**Step 8 of Module 2**</a>).
    ...
    From the left-hand navigation menu, navigate to **Operators** > **OperatorHub**.

    </br>
    ![](_attachments/onprem-5.2.png){: loading=lazy width="400"}
...
4. From the configuration screen, select the following options:
    ...
    - *Version:* `1.13.0`
    - When ready, click the blue **Install** button
...
5. A new page will load, summarizing the settings of the proposed Operator. Verify that the details are correct:
    - *Update channel*: `stable-v1.13`
    - *Version:* `1.13.0`
    - *Installation mode:* `A specific namespace on the cluster`
    - *Installed namespace:* `Operator recommended Namespace: cert-manager-operator`
    - *Update approval:* `Manual`
    
    </br>
    When ready, scroll down to the bottom of the page and click the blue **Install** button. The page will refresh periodically as the Operator is installed on the cluster. This may take several minutes to complete.
    
    - When prompted with a *Manual approval required* splashscreen, click **Approve**
    - Wait until the page refreshes with the message *Installed operator: ready for use* and then click **View Operator** to view the finalized Operator

    </br>
    ![](_attachments/onprem-5.5a.png){: loading=lazy width="400"}
    </br>
    ![](_attachments/onprem-5.5b.png){: loading=lazy width="400"}
    </br>
    ![](_attachments/onprem-5.5c.png){: loading=lazy width="400"}
...
7. On the OCP Dashboard, make sure you are under the **Operators** > **Installed Operators** view.
    ...
    - From the tabs along the top of the page, click **Subscription**
    ...
    ![](_attachments/onprem-5.7.png){: loading=lazy width="400"}

---

8. A new page, *InstallPlan details* for `install-n5l8v`, will load.
    - Click the blue **Preview InstallPlan** button
...
10. **Verify** that `cert-manager` pods are up and running by executing the following command with your Terminal console:
    ...
    ![](_attachments/onprem-5.10.png){: loading=lazy width="400"}
...
16. As before, check on the **status** of the OpenShift pods with the following command:
    ...
    ![](_attachments/onprem-5.16.png){: loading=lazy width="400"}
...
20. **Execute** the following command to verify the status of the install plan:
    ...
    ![](_attachments/onprem-5.20a.png){: loading=lazy width="400"}
    </br>
    ![](_attachments/onprem-5.20b.png){: loading=lazy width="400"}
...
25. Create the **Operator Subscription** for `rhods-operator` within the same project:
    ...
    !!! warning ""
        **ATTENTION:** It will take approximately 1 minute for the Operator installation to complete. Return to the OCP Dashboard and drill down into **Operators** > **Installed Operators** to monitor the progress of the `Red Hat OpenShift AI` Operator deployment.
...
28. **Execute** the following instruction to check the status of pods within the `redhat-ods-applications` project:
    ...
    - `odh-model-controller-0123xyz-1...`
    - `odh-model-controller-0123xyz-2...`
    - `odh-model-controller-0123xyz-3...`
...
29. Now you must edit the `inferenceservice-config` configuration map within the `redhat-ods-applications` project. This is best achieved by using the OCP Dashboard web interface.
    ...
    - In the `metadata.annotations` section (Lines 4-5) of the file, add a new Line 8 which reads `opendatahub.io/managed: 'false'`
...
30. Within the same YAML file, look for Line 379 under the *ingress* section:

    ``` yaml
    "domainTemplate": "{ .Name }-{ .Namespace }.{ .IngressDomain }",
    ```

    **Replace** Line 379 with the following (be sure to include the last `,` character):

    ``` yaml
    "domainTemplate": "example.com",
    ```

    Click **Save** (bottom-left) to finalize the changes to the YAML file. If prompted with a *Managed resource* warning, click **Save** again to confirm.
