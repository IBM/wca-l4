...
20. **Execute** the following command to verify the status of the install plan:
    ...
    !!! warning "No resources found in nvidia-gpu-operator namespace."
        **Troubleshooting:** The `nvidia-gpu-operator` namespace is still being deployed. Wait for 1 minute and then try executing Step 20 again.
...
28. **Execute** the following instruction to check the status of pods within the `redhat-ods-applications` project:
    ...
    !!! warning ""
        ...
        kubeflow-training-operator-659b5dcb99-5t8vt   1/1     Running   0          56s
